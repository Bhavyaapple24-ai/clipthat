...
openssl pkcs12 -export -inkey "$TMP/key.pem" -in "$TMP/cert.pem" \
    -name "$IDENTITY" -out "$TMP/id.p12" -passout pass:macmedal 2>/dev/null

# Import key+cert and pre-authorize codesign to use the key (avoids repeated prompts).
security import "$TMP/id.p12" -k "$KEYCHAIN" -P macmedal -T /usr/bin/codesign
