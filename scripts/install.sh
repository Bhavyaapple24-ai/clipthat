...
tccutil reset ScreenCapture com.clipthat.app >/dev/null 2>&1 || true

echo "▸ Quitting any running copy…"
pkill -f "ClipThat.app" >/dev/null 2>&1 || true
sleep 1

echo "▸ Launching ClipThat…"
open "$ROOT/ClipThat.app"
...
LAST STEP (one time only):
  ...
  2. In Settings ▸ Privacy & Security ▸ Screen & System Audio Recording,
     make sure "ClipThat" is ON.
  3. Click the ◉ menu-bar icon ▸ Quit, then run:  open "ClipThat.app"
